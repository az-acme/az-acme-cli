...
namespace AzAcme.Core.Providers.AzurePrivateDns
{
    public class AzurePrivateDnsZone : IDnsZone
    {
        private readonly ILogger logger;
        private readonly PrivateDnsManagementClient client;
        private ResourceId azureDnsResource;
        private string zoneName;

        public AzurePrivateDnsZone(ILogger logger, PrivateDnsManagementClient client, string azureDnsZoneResourceId, string? zoneOverride)
        {
        ...
        private async Task RemoveTxtRecord(DnsChallenge challenge)
        {
            ...
            var recordSets = await client.RecordSets.ListByTypeAsync(azureDnsResource.ResourceGroupName, azureDnsResource.Name, Microsoft.Azure.Management.PrivateDns.Models.RecordType.TXT);
            ...
            else
            {
                ...
                await client.RecordSets.DeleteAsync(azureDnsResource.ResourceGroupName, azureDnsResource.Name, RecordType.TXT, challenge.TxtRecord);
            }
        }
        ...
        private async Task UpdateTxtRecord(DnsChallenge challenge)
        {
            ...
            if (records == null)
            {
                ...
                var result = client.RecordSets.CreateOrUpdate(azureDnsResource.ResourceGroupName, azureDnsResource.Name, RecordType.TXT, challenge.TxtRecord, records);
            }
            else
            {
                if (!(records.TxtRecords.Any(txt => txt.Value.Any(val => val == challenge.TxtValue))))
                {
                    ...
                    var result = client.RecordSets.Update(azureDnsResource.ResourceGroupName, azureDnsResource.Name, RecordType.TXT, challenge.TxtRecord, records);
                }
                else
                {
